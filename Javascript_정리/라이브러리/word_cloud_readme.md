...
-- 어떤 단어가 성공적으로 place될 때 마다, word 이벤트가 dispatch(보내진)다. 

Registered listeners / are called / with a single argument: the word object that
has been placed.

단일한 argument(place된 word object.)를 통해서 등록된 listener들이 호출된다.  

An "end" event is dispatched / when the layout has finished attempting to place
all words.  

그리고, "end" 이벤트는 레이아웃이 모든 단어들을 place하는걸 시도하는게 끝났을 때 dispatch된다.

Registered listeners are called with two arguments: an array of the
...
This initialises various attributes on the word
...
collisions with all previously-placed words.  

If a collision is found, it tries
